 ...
 *   --class NAME           Formula class name (default: OzAcp)
 *   --help, -h
 */
...
function renderFormula({
  ...
  // oz ships as a cask on warpdotdev/homebrew-warp (not a formula).
  // Homebrew formulas cannot `depends_on cask:`; use a Requirement that
  // checks for `oz` on PATH and points installers at the Warp cask.
  // https://github.com/warpdotdev/homebrew-warp/blob/main/Casks/oz.rb
  return `# typed: false
...
async function main() {
  ...
  const name = pkg.name || "oz-acp";
  const repo = resolveRepo(opts.repo);
  const homepage = pkg.homepage || `https://github.com/${repo}`;
  const desc = pkg.description || "ACP stdio adapter for Warp Oz CLI";
